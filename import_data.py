...
def create_spectrogram(verbose=0, mode=None):
    if mode == "Train":
        ...
        filename_metadata = "Dataset/fma_metadata/raw_tracks.csv"
        tracks = pd.read_csv(filename_metadata, header=2, low_memory=False)
        tracks_array = tracks.values
        tracks_id_array = tracks_array[:, 0]
        tracks_genre_array = tracks_array[:, 27]
        # tracks_genre_array = list(tracks_genre_array)

        # Extract only genre_title from the tracks_genre_array
        result = []
        for index, i in enumerate(tracks_genre_array):
            if (isinstance(i, str)):
                result.append(json.loads(i.replace("'", "\""))
                              [0]['genre_title'])
            else:
                result.append("")
        tracks_genre_array = np.array(result)
        tracks_id_array = tracks_id_array.reshape(tracks_id_array.shape[0], 1)

        tracks_genre_array = tracks_genre_array.reshape(
            tracks_genre_array.shape[0], 1)
        ...
        for d in directories:
            ...
            for f in file_names:
                print(re.search('fma_small/.*/(.+?).mp3', f))
        ...
        for f in file_names:
            ...
            librosa.display.specshow(mel, cmap='gray_r')
            plt.savefig("Test_Spectogram_Images/"+test_id +
                        ".jpg", bbox_inches=None, pad_inches=0)
            plt.close()
        return
