    ...
    if mode == "Train":
        ...
        for f in filenames:
            genre_variable = re.search(
                'Train_Spectogram_Images/.*_(.+?).jpg', f).group(1)
            img = Image.open(f)
            subsample_size = 128
            ...
            for i in range(int(number_of_samples)):
                ...
                img_temporary = img.crop(
                    (start, 0., start + subsample_size, subsample_size))
                img_temporary.save("Train_Sliced_Images/" +
                                   str(counter)+"_"+genre_variable+".jpg")
                counter = counter + 1
        return
        ...
        for f in filenames:
            song_variable = re.search(
                'Test_Spectogram_Images/(.+?).jpg', f).group(1)
            img = Image.open(f)
            subsample_size = 128
            width, height = img.size
            number_of_samples = width / subsample_size
            for i in range(int(number_of_samples)):
                ...
                img_temporary = img.crop(
                    (start, 0., start + subsample_size, subsample_size))
                img_temporary.save("Test_Sliced_Images/" +
                                   str(counter)+"_"+song_variable+".jpg")
                counter = counter + 1
        return
