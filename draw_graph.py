...
train_x, train_y, test_x, test_y, n_classes, genre = load_dataset(
...
fig['layout']['xaxis'].update(title="Number of Epochs", range=[min(epochs), max(epochs)], dtick=len(
    epochs)/10, showline=True, zeroline=True,  mirror='ticks', linecolor='#636363', linewidth=2)
fig['layout']['yaxis'].update(title="Accuracy", range=[
                              0, 1], dtick=0.1, showline=True, zeroline=True, mirror='ticks', linecolor='#636363', linewidth=2)
...
fig['layout']['xaxis'].update(title="Number of Epochs", range=[min(epochs), max(epochs)], dtick=len(
    epochs)/10, showline=True, zeroline=True,  mirror='ticks', linecolor='#636363', linewidth=2)
fig['layout']['yaxis'].update(title="Loss", dtick=0.1, showline=True,
                              zeroline=True, mirror='ticks', linecolor='#636363', linewidth=2)
...
labels = ["Hip-Hop", "International", "Electronic",
          "Folk", "Experimental", "Rock", "Pop", "Instrumental"]

trace = go.Heatmap(z=confusion_matrix, x=labels, y=labels,
                   reversescale=False, colorscale='Viridis')
data = [trace]
layout = go.Layout(
